...
function progressMessage(seconds: number) {
  if (seconds < 5) return 'Uploading ledgers and starting parsers';
  if (seconds < 15) return 'Extracting PDF/Excel rows and balances';
  if (seconds < 35) return 'Running deterministic matching and exception checks';
  if (seconds < 75) return 'Reviewing difficult rows with AI where enabled';
  return 'Generating Excel workbook and summary exports';
}
function progressPercent(seconds: number) {
  return Math.min(92, 8 + seconds * 1.4);
}
